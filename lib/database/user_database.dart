...
class UserDatabase{
  ...
  Future updateMenstrualDetails(int mLength,int pLength,List<DateTime>lDate)async{
    await collectionReference.doc(uid).update({
    ...
    return user.map((DocumentSnapshot snapshot) {
      ...
      dynamic dt = Map<String,dynamic>.from(snapshot.data());
        ...
        if(snapshot.exists == false){
          AppUser user = AppUser(uid:uid,name: "User",phoneNumber: phone,address:"",menstrualLength: 3,periodLength: 28,lastMenstruation: [],requestIds: []);
          updateUserData(user);
          return user;
        }else{
          return null;
